...
class Post(models.Model):
    ...
    # add a favourites field which will hold
    # the IDs of all Users which favourited this post
    favourites = models.ManyToManyField(User, related_name='favourite',
                                        default=None, blank=True)
    updated_on = models.DateTimeField(auto_now=True)
    # make categories very difficult to delete
    # set default category to Miscellaneous (1)
...
class Comment(models.Model):
    ...
    def __str__(self):
        # Adjust the format of the date to only include year, month and day
        shortened_date = self.created_on.strftime("%Y-%m-%d")
        return f"Comment: '{self.body}' by {self.author} " \
               f"commented on {shortened_date}"
